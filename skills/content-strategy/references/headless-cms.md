...
Use the CMS as the data source for programmatic pages. Store structured data (FAQs, comparisons, city pages) as content types and generate pages from queries. See the **programmatic-seo** skill.

### Content Strategy

The content types you model here should mirror the content pillars and topic clusters in your plan — model a type per pillar's recurring format (guide, comparison, case study) so the editorial roadmap and the CMS schema stay in sync. See the **content-strategy** skill.

### Keyword Coverage

Blog Post and Landing Page types should carry the target keyword and buyer stage as fields, so every published piece traces back to the keyword plan. See the **keyword-strategy** skill.
